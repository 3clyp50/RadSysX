...
- `/worklist` may expose local imaging import and imported-study inspection/analysis controls when backend platform config enables them; native picker, browser input, and drag-and-drop imports plus asset summaries, technical analysis, NIFTI slice navigation including paired `.hdr/.img`, and preview thumbnails must go through the backend local imaging contract.
