...
- `src/client.ts` owns the browser client for backend local imaging import, imported-study asset summaries/previews, API URL resolution, and relative-path preservation for folder/DICOMDIR uploads from browser `webkitRelativePath` and Electron `radsysxRelativePath`.
