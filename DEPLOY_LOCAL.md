...
`npm run desktop:smoke:local-start` starts Electron at `/viewer/local`, verifies OHIF's local file/folder controls are visible without a RadSysX bootstrap card, loads a synthetic DICOM through OHIF's local file input, routes to `/viewer/dicomlocal?datasources=dicomlocal`, and asserts that OHIF paints a nonblank canvas without a governed launch.

`npm run desktop:smoke:local-start-drop` starts Electron at the same OHIF local screen, drops a synthetic DICOM directly onto it, forwards the drop into OHIF's local file input, routes to `/viewer/dicomlocal?datasources=dicomlocal`, and asserts that OHIF paints a nonblank canvas without a governed launch.
