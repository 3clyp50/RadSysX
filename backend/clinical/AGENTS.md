...
- Keep local imaging import, safe ZIP archive expansion, safe imported-study asset summaries/previews/technical analysis, paired NIFTI `.hdr/.img` linking, TIFF SVG header previews, and local DICOMweb serving backend-owned through `local_imaging.py`; imported files must stay in private local storage, not public frontend paths.
