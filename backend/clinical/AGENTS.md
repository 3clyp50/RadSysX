...
- Keep local imaging import backend-owned through `local_imaging.py`; imported files must stay in private local storage, not public frontend paths.
...
- Put local DICOM/DICOMDIR/NIFTI/fallback-file import detection and storage logic in `local_imaging.py`.
