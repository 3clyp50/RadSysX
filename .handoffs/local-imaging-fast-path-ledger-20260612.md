...
- The desktop window now opens `/viewer/?local=1` by default instead of the root shell, then the OHIF bootstrap strips the visible `local` query and leaves the user at `/viewer/` with `Open local study` focused as the primary local import action plus first-screen drag/drop for no-click local file import.
...
- The OHIF bootstrap has one narrow no-launch exception: when Electron opens `/viewer/?local=1` and exposes `window.radsysxDesktop.importLocalImaging`, the viewer can auto-establish the seeded local session, show a compact local import card, focus the single primary `Open local study` action, accept dropped files/folders directly on that card, and keep the user in the viewer surface rather than asking them to choose between product modes.
- The OHIF local-start card keeps `Open local study` as the primary action and demotes folder selection to `Choose folder`; both actions are backed by the existing Electron native import bridge, while first-screen drag/drop submits browser `File` objects directly to `POST /api/local-imaging/import` with `relativePaths` preserved through the backend contract.
...
- The desktop local-start smoke can validate the current desired first experience: Electron starts on `/viewer/`, the OHIF local import card is present without navigating through the root shell/worklist first, `Open local study` is focused as the primary action, imported DICOM opens in OHIF through the governed launch contract, the visible URL remains clean, viewer-origin local DICOMweb/workspace reads succeed, and the synthetic DICOM paints a nonblank canvas.
- The desktop local-start drag/drop smoke can validate the same first experience with no button click: Electron starts on `/viewer/`, synthetic files are dropped directly onto the OHIF local-start card, the backend import contract receives preserved relative paths, imported DICOM opens in OHIF through the governed launch contract, and the synthetic DICOM paints a nonblank canvas.
- The desktop local-start non-DICOM smoke can validate the same first experience for a NIFTI/NRRD/image/ZIP-only selection: Electron starts on `/viewer/`, import runs from `Open local study` on the OHIF local-start card, the app falls back to `/worklist`, the local NIFTI/NRRD/image asset panel auto-opens, previews load, a NIFTI preview axis changes, backend technical analysis runs, and the local row does not expose an OHIF viewer action.
