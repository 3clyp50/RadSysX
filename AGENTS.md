...
- Fast local desktop path:
  ...
  - Standalone and governed OHIF viewer layouts include a RadSysX AI right-sidebar panel. For now that panel is frontend-only: it keeps chat messages locally, offers a bottom composer with text and voice-input affordances, and lets users attach ROI/segmentation/measurement context chips through `@` without introducing backend AI contracts.
